    ...
    def submit(self):
      ...
      # print('Decrypted:', id1_decrypted.decode('utf-8'))
       ...
       try:
            ...
            con2= pymysql.connect(host="localhost",user="root",password="123456@abc",database="database1")
            cur = con.cursor()
            cur1 = con1.cursor()
            cur2 = con2.cursor()
            if (self.cmb_quest.get() == "Read"):
                ...
                cur.execute("insert into anz.departments(dept_id,dept_name) values(%s,%s)",(var_dept_id,dept_name_encrypted))
                cur1.execute("insert into database2.departments(dept_id,dept_name) values(%s,%s)",(var_dept_id,var_dept_name))
                cur2.execute("insert into database1.departments(dept_id,dept_name) values(%s,%s)",(dept_id_encrypted,var_dept_name))
                self.keyStore(var_dept_id,keyPair)
                cur.execute("insert into anz.employees(name,dept_id,id,salary,branch_name) values(%s,%s,%s,%s,%s)", (name_encrypted,dept_id_encrypted,var_id1,salary_encrypted,branch_name_encrypted) )
                ...
                cur2.execute("insert into database1.employees(name,dept_id,id,salary,branch_name) values(%s,%s,%s,%s,%s)", (name_encrypted,var_dept_id,id1_encrypted,var_salary,var_branch_name) )
